...
class ChatGPTBot(Bot, OpenAIImage):
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            if split_strings[0] in prefix:
                ...
                logger.debug("[CHATGPT] session query={}".format(session.messages))
                reply_content = self.reply_text(session, 1, api_key, args=new_args)
                logger.info("正在调用GPT-4")
            else:
                reply_content = self.reply_text(session, 0, api_key, args=new_args)
            logger.debug(
    ...
    def reply_text(self, session: ChatGPTSession, model_selection, api_key=None, args=None, retry_count=0) -> dict:
        """
